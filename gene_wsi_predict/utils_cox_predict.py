...
@torch.no_grad()
def predict(model, topK, criterion, data_loader, json_path, save_attn_dir, reg_loss=False, train_flag=0, contrastive_loss_flag=0):
    model.eval()
    ...
    for step, data in enumerate(data_loader):
        ...
        if train_flag == 0:
            if contrastive_loss_flag==1:
                ...
                gene2wsi_feature, pred, attn = model(wsi_features, gene_features,)
                ...
                #import pdb;pdb.set_trace()
                pred, attn = model(wsi_features, gene_features,)
                #mean_head_attn = torch.mean(attn, dim=1)
                gene2wsiloss=0
        elif train_flag == 1:
        ...
        for head_idx in range(attn.shape[1]):
            if not os.path.exists(os.path.join(save_attn_dir, 'head{}'.format(str(head_idx)))):
                os.makedirs(os.path.join(save_attn_dir, 'head{}'.format(str(head_idx))))
            single_head_attn = attn[:,head_idx,:,:] 
            for idx, attn_tensor in enumerate(single_head_attn):
                torch.save(attn_tensor.cpu(), os.path.join(os.path.join(save_attn_dir, 'head{}'.format(str(head_idx))), file_name[idx].replace('.txt', '.pth')))
        loss= loss.mean()
        accu_loss += loss
