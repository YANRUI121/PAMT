...
class Gene_Guided_Transformer_Fusion(nn.Module):
    ...
    def forward(self, x1, x2):
        #x1 is wsi: 500 * 256; x2 is gene: 186*256
        # [batch_size, num_patches + 1, total_embed_dim]
...
class VisionTransformer(nn.Module):
    def __init__(self, wsi_patches=500, gene_patches=186, embed_wsi_dim=2048, embed_gene_dim=256, num_classes=1000, 
        ...
        #self.labels = torch.zeros(1, gene_patches, wsi_patches)
        #self.labels[:,:topK] = 1
        
        self.patch_embed = embed_layer(embed_dim=embed_wsi_dim)

        self.gene_embed = EmbedReduction(in_features=5245, hidden_features=1000, out_features=self.embed_gene_dim, act_layer=act_layer, drop=drop_ratio)
        ...
        self.blocks_wsi = nn.Sequential(*[
            Block(dim=self.embed_wsi_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                  drop_ratio=drop_ratio, attn_drop_ratio=attn_drop_ratio, drop_path_ratio=dpr[i],
                  norm_layer=norm_layer, act_layer=act_layer)
            for i in range(depth_fusion)
        ])
        ...
        self.blocks_gene = nn.Sequential(*[
            Block(dim=self.embed_gene_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                  drop_ratio=drop_ratio, attn_drop_ratio=attn_drop_ratio, drop_path_ratio=dpr[i],
                  norm_layer=norm_layer, act_layer=act_layer)
        ...
        self.wsi_embed_reduction = EmbedReduction(in_features=self.embed_wsi_dim, hidden_features=640, out_features=self.embed_gene_dim, act_layer=act_layer, drop=drop_ratio)
        self.gene_guided_wsi_fusion = Gene_Guided_Transformer_Fusion(dim=self.embed_gene_dim)
        #self.gene_guided_wsi_fusion = nn.Sequential(*[
        #    Gene_Guided_Transformer_Fusion(dim=self.embed_gene_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
    ...
    def forward(self, x_wsi, x_gene):
        ...
        # 186 * 2
        #sorted_gen2wsi_feature, _ = torch.sort(gene2wsi_feature, descending=True, dim=2)
        #gene2wsi_loss = F.cross_entropy(sorted_gen2wsi_feature, self.labels, reduction='mean')
        #top2_values, _ = torch.topk(gene2wsi_feature, k=2, dim=2)
        #top2_values = F.log_softmax(top2_values, dim=-1)
        ##top2_values = F.log_softmax(top2_values/self.logit_scale, dim=-1)
        ...
        return gene2wsi_feature, pred_head
        return gene2wsi_feature, pred_head, attn
